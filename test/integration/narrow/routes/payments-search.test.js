...
describe('payments-search routes', () => {
  ...
  test.each([
    ['no search string', '/v1/payments/search'],
    ['empty search string', '/v1/payments/search?searchString='],
    ['search string exceeds 32 characters', `/v1/payments/search?searchString=${'a'.repeat(33)}`]
  ])('GET /v1/payments/search should return 400 if %s', async (_label, url) => {
    const response = await server.inject({ method: 'GET', url })
    expect(response.statusCode).toBe(400)
  })

  test('GET /v1/payments/search should return validation error message if no search string', async () => {
